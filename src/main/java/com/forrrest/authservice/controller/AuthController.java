...
public class AuthController {
    ...
    @Operation(summary = "토큰 갱신(쿠키 기반 refreshToken)",
            description = "헤더에 입력된 리프레시 토큰을 사용하여 새로운 액세스 토큰을 발급받습니다. Profile ID를 입력하지 않은 경우 default Profile 토큰을 발급합니다.",
            security = {@SecurityRequirement(name = CookieUtils.COOKIE_NAME), @SecurityRequirement(name = "bearer-token")})
    @PostMapping("/refresh")
    public ResponseEntity<AuthResponse> refresh(
        ...
        AuthResponse authResponse = authService.refreshToken(refreshTokenRequest, profileId);
        ResponseCookie refreshCookie = CookieUtils.createRefreshTokenCookie(
                authResponse.getUserToken().getRefreshToken(),
                authResponse.getUserToken().getExpiresIn()
        );
    ...
    public ResponseEntity<Void> logout(HttpServletResponse response) {
        ...
        //todo refreshToken 삭제 코드 추가, accessToken redis 블랙리스트 추가
